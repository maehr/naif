...
Use a news post for updates, progress reporting, findings, or opinion pieces in British English.
...
- Do not add an explicit `## Lead` heading. Use a lead paragraph block instead:
  ```markdown
  ::: {.lead}
...
- British English is preferred for all non-repost event pages.
...
- `categories`: exactly one: `Organised by NAIF`, `NAIF participation`, or `NAIF-related topic`
- `tags`: exactly 3 tags
- `proofread`: `true` or `false`
- `image`: local image path (used for listing cards)
