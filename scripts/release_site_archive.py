...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for release archive creation."""
...
def sanitize_tag(tag: str) -> str:
    """Convert a release tag into a filesystem-safe archive suffix."""
...
def default_tag() -> str:
    """Return a timestamp fallback when no release tag is provided."""

    return datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ")


def ensure_site_dir(site_dir: Path) -> None:
    """Fail fast when the rendered site directory does not exist."""
...
def remove_existing_archives(archive_dir: Path, archive_name: str) -> None:
    """Keep only the current release archive in the tracked output directory."""
...
def build_archive(site_dir: Path, archive_path: Path) -> None:
    """Write a ZIP archive containing the rendered site contents."""
...
def stage_archive_dir(archive_dir: Path) -> None:
    """Stage the generated archive for commit when requested."""

    subprocess.run(["git", "add", "-A", str(archive_dir)], cwd=REPO_ROOT, check=True)


def main() -> int:
    """Create a release archive and optionally stage it for commit."""
