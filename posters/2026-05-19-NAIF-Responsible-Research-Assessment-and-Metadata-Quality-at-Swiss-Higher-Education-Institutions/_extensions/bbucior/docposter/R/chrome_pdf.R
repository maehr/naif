...
chrome_pdf <- function(
  ...
  # Waiting for page load is a difficult problem and known issue in scripted/headless Chrome.
  ...
  args <- c(args, sprintf("--virtual-time-budget=%s", 30000))
  ...
  if (res$status != 0 || res$timeout) {
    pdf_written <- file.exists(pdf_path) && is.finite(file.info(pdf_path)$size) && file.info(pdf_path)$size > 0
    if (!pdf_written) {
      stop(paste0("Crash or timeout from Chrome\n", res$stderr))
    }
    warning(paste0("Chrome exited non-zero after writing the PDF; continuing.\n", res$stderr))
  }
